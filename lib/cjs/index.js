...
const ReactInputVerificationCode = ({ autoFocus = false, inputProps, length = 4, onChange = () => null, onCompleted = () => null, placeholder = '·', type = 'number', value: defaultValue = '', }) => {
    ...
    const [values, setValues] = React.useState(fillValues(defaultValue));
    const [focusedIndex, setFocusedIndex] = React.useState(-1);
    const inputsRefs = React.useMemo(() => new Array(length).fill(null).map(() => React.createRef()), [length]);
